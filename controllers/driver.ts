...
export const loginDriver = async (req: Request, res: Response, next: NextFunction): Promise<Response | ErrorResponse | void> => {
  ...
  const token = jwt.sign({ _id: driver.id }, process.env.JWT_PRIVATE_KEY!, {
    expiresIn: '30d',
  });

  const response = SuccessResponse(res, 200, 'Driver logged successfully', token);
  return response;
};
